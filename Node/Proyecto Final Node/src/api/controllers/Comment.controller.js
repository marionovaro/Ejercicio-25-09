...
const create = async (req, res, next) => {
    try {
        ...
        const body = req.body
        const customBody = { //? creamos un customBody para añadirle el creador que nos lo da el token y la location que nos la da la url. a parte del contenido que lo damos en el body
            creator: creator,
            location: location,
        ...
        const updateEleven = await Eleven.findByIdAndUpdate(location, //? ----- hacemos que sea recíproco, y que se cree el comentario en el eleven
            {$push: {comments: saveComment._id}})
        return res
            .status(saveComment ? 200 : 404)
            .json(saveComment ? saveComment : "Error en el guardado del comentario")
    } catch (error) {
        return next(setError(500, error.message || "Error general al crear tu comentario ❌"))
    }
}
...
const getById = async (req, res, next) => {
    try {
        const {id} = req.params;
        const commentById = await Comment.findById(id); //? cogemos el elemento (eleven) identificandola a través del id, que es único
        return res
            .status(commentById ? 200 : 404)
            .json(commentById ? commentById : "no se ha encontrado un comentario con ese id ❌");

    } catch (error) {
        return next(setError(500, error.message || "Error general al buscar comentario a través de ID ❌"))
    }
};
...
const getAll = async (req, res, next) =>  {
    try {
        ...
        const allComments = await Comment.find({location: location}) //? ------------- el find() nos devuelve un array con todos los elementos de la colección del BackEnd, es decir, Ttodos los comments que tengan location en el id que hemos puesto
        if (elevenById) {
            return res 
                .status(allComments.length > 0 ? 200 : 404) //? ---- si hay equipos en la db (el array tiene al menos 1 elemento), 200 o 404
                .json(allComments.length > 0 ? allComments : `No se han encontrado comentarios en ${elevenById.name} en la DB ❌`); 
        } else { return res.status(404).json(`El eleven porporcionado en la url no existe ❌`)}

    } catch (error) {
        return next(setError(500, error.message || `Error general al buscar todos los comentarios del 11 ideal indicado ❌`))
    }
};
...
const deleteComment = async (req, res, next) => {
    try {
        ...
        return next(setError(500, error.message || "Error general al eliminar tu comentario ❌"))
    }
}; 
